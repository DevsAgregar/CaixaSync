...
def cruzar_planilhas_movimentacao(arquivo_formatado: str, arquivo_movimentacoes: str, pasta_saida: str) -> None:
    ...
    df_formatada['Valor'] = df_formatada['Valor'].replace(',', '.', regex=True).astype(float).round(2)
    ...
    df_mov['Valor (R$)'] = df_mov['Valor (R$)'].replace(',', '.', regex=True).astype(float).round(2)
    ...
    for conta in contas:
        ...
        df_conta['Valor'] = df_conta['Valor (R$)'].apply(lambda x: f"{x:,.2f}".replace('.', ','))
        ...
        df_conta['CPNJ/CPF Cliente/Fornecedor'] = ''
        ...
        df_conta = df_conta[[
            ...
            'CPNJ/CPF Cliente/Fornecedor', 'Centro de Custo', 'Observações'
        ]]

        nome_arquivo = f"{sanitizar_nome_arquivo(conta)}.xlsx"
        caminho_arquivo = os.path.join(pasta_saida, nome_arquivo)
        df_conta.to_excel(caminho_arquivo, index=False)
